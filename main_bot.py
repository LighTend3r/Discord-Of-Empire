...
async def check_if_aventurier(ctx):
	verif = any(role.id == id_role_aventrier for role in ctx.message.author.roles)
	if not verif:
...
mongo_url = "mongodb+srv://LighTender:"+ MDP_MONGODB + "@cluster0.2mtlq.mongodb.net/BotDiscordRolePlay?retryWrites=true&w=majority"
...
id_role_aventrier = 858347087648391209
...
async def on_member_join(member):
	...
	if collection.count_documents({"_id": member_id}) == 0:
		role_tuto = discord.utils.find(
			lambda r: r.id == id_role_tuto, member.guild.roles)
		...
		role = discord.utils.find(
			lambda r: r.id == id_role_aventrier, member.guild.roles)
...
async def on_raw_reaction_add(payload):  # sourcery no-metrics
	...
	if payload.emoji.name == "✅" and payload.channel_id == channel_reception_id and payload.message_id == message_reception_id:
		...
		role = discord.utils.find(
			lambda r: r.id == id_role_aventrier, author.guild.roles)
...
@bot.command(name='lvl', aliases=["niveau", "level"], help="Cette commande revoie tout simplement votre Niveau, ainsi que votre Xp")
async def lvl(ctx):
	if not await check_if_aventurier(ctx):
		return
...
@bot.command(name='godfather', aliases=["parrain"])
...
async def wood(ctx):  # sourcery no-metrics
	...
	await ctx.channel.send(f"{message}\n{message2}")
	await ctx.channel.send(f"https://tenor.com/view/countdown-counter-seconds-30seconds-timer-gif-20049533", delete_after=30)
...
@bot.command(name='cooldown', help="Cette commande te permet de connaître le cooldown du !week et !vote")
...
@bot.command(name='géode', aliases=["geode"], help="Cette commande te permet d'ouvrir des géode, ainsi que le "+"%"+" de chance que tu as d'avoir chaque objet à l'interieur")
...
@bot.command(name='inventory', aliases=["inv","inventaire"], help="Cette commande te permet de voir tout ce que tu as au niveau matériaux, ainsi que tes outil, hache et pioche")
async def inv(ctx):
	if not await check_if_aventurier(ctx):
		return
...
@bot.command(name='buy', aliases=["acheter"], help="Cette commande permet d'acheter certain item, pour conaître tout ce que vous pouvais acheter avec cette commande, faite tout simplement !buy, elle évolue en fonction de votre avancé sur le jeu")
async def buy(ctx, arg="r"):
	...
	if arg == "r":
		...
		await ctx.channel.send(f"Vous venez d'obtenire votre **rubis pur** !, vous pouvez l'incrusté dans votre pioche avec `!inlaid pure_sapphire`, {ctx.author.mention}")
...
@bot.command(name='upgrade', aliases=["up"], help="Cette commande permet d'améliorer certain item, pour conaître tout ce que vous pouvais améliorer avec cette commande, faite tout simplement !upgrade, elle évolue en fonction de votre avancé sur le jeu")
...
@bot.command(name='item', aliases=["objet"], help="Cette commande permet de connaître les spécificité de chaque item, pour conaître tout les objets consultable, faite tout simplement !item")
...
@bot.command(name='potion', help="Cette commande permet de voir et d'utiliser les potions")
...
@bot.command(name='axe', aliases=["hache"], help="Cette commande permet de connaître exactement combien de bois tu récupaire avec tout les bonus que tu as")
async def axe(ctx):
	if not await check_if_aventurier(ctx):
		return
	author_id = ctx.author.id
...
@bot.command(name='pickaxe', aliases=["pioche"], help="Cette commande permet de connaître exactement combien de minerais tu récupaire avec tout les bonus que tu as")
...
@bot.command(name='sword', aliases=["épée"], help="Cette commande permet de connaître ton épée")
async def sword(ctx):
	if not await check_if_aventurier(ctx):
		return
	author_id = ctx.author.id
...
@bot.command(name='invitation', aliases=["invite"], help="Cette commande te permet de connaître le nombre de personne que tu as invitée")
...
@bot.command(name='stats', aliases=["statistique", "statistiques", "stat"], help="Cette commande te permet de voir quelque statistique sur ton compte")
async def stat(ctx):
	verif = any(role.id == id_role_aventrier for role in ctx.message.author.roles)
	if not verif:
		channel_reception = discord.utils.find(
...
@bot.command(name='command', aliases=["commande"], help="Cette commande te permet de voir quelque que commande au cas ou tu en a oublié certaine")
...
@bot.command(name='house', aliases=["home", "maison"], help="Cette commande te permet de voir ta maison, acheter une maison est indispensable pour rejoindre t'a faction")
...
@bot.command(name='faction', help="Cette commande te permet dans un premier temps de choisir t'as faction et dans un second de voir tout ce que tu as besoin de voir sur ta faction")
...
@bot.command(name='bank', aliases=["banque"], help="Cette commande te permet de déposé des matériaux à a banque de ta faction, quand votre faction a remplie la banque, alors la faction passe au niveau supérieur")
...
@bot.command(name='furnace', aliases=["smelt,four"], help="Cette commande te permet d'échangé certain matériaux contre d'autre, il est achetable dans le !buy quand vous avais acquis une maison")
...
@bot.command(name='inlaid', aliases=["incrusté,incruste,incruster"], help="Cette commande te permet d'incrusté des pierres précieuses pur dans ta pioche, mais attention, changé de pioche fait dispaitre la pierre précieuse pur'")
...
# 4 fois !mine
@bot.command(name='hour', aliases=["h", "heure"], enabled=False)
...
# 2 fois !mine
@bot.command(name='vote', aliases=["v"], help="Cette commande te permet de voté pour ton serveur préférais, et en contreparti de recevoir une récompense alléchante")
async def vote(ctx):
	if not await check_if_aventurier(ctx):
		return
	author_id = ctx.author.id
...
@bot.command(name='day', aliases=["jour", "j"], enabled=False)  # 60 fois !mine
...
@bot.command(name='week', aliases=["semaine"])  # 420 fois !mine
async def week(ctx):
	if not await check_if_aventurier(ctx):
		return
	author_id = ctx.author.id
...
@bot.command(name='rank', aliases=["leaderboard", "classement", "top"])
...
@bot.command(name='aventure', aliases=["adventure", "av"])
...
@bot.command(name='expedition', aliases=["expédition"])
...
@bot.command(name='biome', aliases=["biomes"])
